 ...
 * 经纬度纠偏工具类 [一句话功能简述]
 * 地图纠偏算法
 * 无需调用地图厂商服务，真实产品使用的，目前只验证高德地图以及使用高德数据的地图（如MapABC等），
 * 百度地图和谷歌地图的纠偏没验证过，见谅。
 * <p>
 * [功能详细描述]
 * <p>
...
public class MapFix {
	...
	public double[] fix(double x, double y) {
		...
		try {
			...
			double num4 = 0.0;
			double num5 = 0.0;
			Point point = wgtochina_lb(1, (int) num, (int) num2, (int) num5, (int) num3, (int) num4);
	...
	private double Transform_yj5(double x, double y) {
		double num = ((((300.0 + (1.0 * x)) + (2.0 * y)) + ((0.1 * x) * x)) + ((0.1 * x) * y)) + (0.1 * Math.sqrt(Math.sqrt(x * x)));
	...
	private Point wgtochina_lb(int wg_flag, int wg_lng, int wg_lat, int wg_heit, int wg_week, int wg_time) {
		Point point = null;
		if (wg_heit <= 0x1388) {
			double num = wg_lng;
			num /= 3686400.0;
